...
<body>
  ...
  <script>
    ...
    document.getElementById('province-select').addEventListener('change', (e) => {
      ...
      const fetchUrl = '/provinces/' + province;
    ...
    document.getElementById('file-select').addEventListener('change', (e) => {
      ...
      if (!district || !province) {
        jsonViewer.value = '';
        updateApiPath('/provinces/' + province);
        return;
      }
      ...
      const fetchUrl = district === '__PROVINCE__' 
        ...
        : '/provinces/' + province + '/' + district + '/data';
...
const app = new Elysia()
  ...
  // 2. Get Districts (Folders inside Province)
  .get("/provinces/:province", async ({ params: { province }, set }) => {
    try {
      const dir = join(DATA_DIR, province);
      ...
      const hasProvinceFile = files.some(
        (d) => d.isFile() && d.name.endsWith(".geojson"),
      );
      return { districts, hasProvinceFile };
    } catch (error) {
      set.status = 404;
      return { error: `Province folder '${province}' not found.` };
    }
  })

  // 3a. Get Province-level GeoJSON File
  .get("/provinces/:province/data", async ({ params: { province }, set }) => {
    try {
      const dir = join(DATA_DIR, province);
      const files = await readdir(dir, { withFileTypes: true });
      const geojsonFile = files.find(
        (f) => f.isFile() && f.name.endsWith(".geojson"),
      );
      if (!geojsonFile) throw new Error();
      const content = await readFile(join(dir, geojsonFile.name), "utf-8");
      return JSON.parse(content);
    } catch (error) {
      set.status = 404;
      return { error: "No province-level .geojson file found." };
    }
  })

  // 3b. MERGE MULTIPLE GEOJSON FILES INTO ONE FEATURE COLLECTION
  .get(
    "/provinces/:province/:district/data",
    async ({ params: { province, district }, set }) => {
      try {
        const dir = join(DATA_DIR, province, district);
        const files = await readdir(dir, { withFileTypes: true });

        // 1. Find all .geojson files in the directory
        const allGeojsonFiles = files.filter(
          (f) => f.isFile() && f.name.endsWith(".geojson"),
        );

        if (allGeojsonFiles.length === 0) throw new Error("No GeoJSON files");

        // 2. Filter out the "Master" district file to avoid duplicating data
        const masterFileName = `${district}.geojson`;
        let filesToRead = allGeojsonFiles.filter(
          (f) => f.name !== masterFileName,
        );
        // 3. Fallback: If this folder ONLY has a master file (no sub-districts), use it
        if (filesToRead.length === 0) {
          filesToRead = allGeojsonFiles.filter(
            (f) => f.name === masterFileName,
          );
        }

        // Read the selected files simultaneously for maximum speed
        ...
        // Return a perfectly formatted single FeatureCollection
        return {
          type: "FeatureCollection",
          features: fileContents,
        };
      } catch (error) {
        set.status = 404;
        return {
          error: `No .geojson files found inside the '${district}' folder.`,
        };
      }
    },
  )
  ...
  .get(
    "/provinces/:province/:district/:file",
    async ({ params: { province, district, file }, set }) => {
      try {
        const dir = join(DATA_DIR, province, district);
        const files = await readdir(dir, { withFileTypes: true });

        // 1. Find all .geojson files in the directory
        const geoFiles = files.find((f) => f.isFile() && f.name === file);

        if (!geoFiles) throw new Error();

        // 2. Read the selected file
        const content = await readFile(join(dir, geoFiles.name), "utf-8");
        const data = JSON.parse(content);
